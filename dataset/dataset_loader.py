...
class NYURaw(object):
  ...
  def __init__(self,
               ...
               img_width=416,
               seq_length=3):
    static_frames_file = 'dataset/nyu/static_frames.txt'
    test_scene_file = 'dataset/nyu/test_scenes_' + split + '.txt'
  ...
  def collect_train_frames(self):
    ...
    all_frames = []
    ls = os.listdir(self.dataset_dir)
    for fn in ls:
      if os.path.isdir(os.path.join(self.dataset_dir,fn)):
        self.date_list.append(fn)
    print(self.date_list)
    for date in self.date_list:
      ls = os.listdir(os.path.join(self.dataset_dir,date))
      rgb_list = []
      for fn in ls:
        if (fn[:2]=='r-'):
          rgb_list.append(date+' '+os.path.join(self.dataset_dir,date,fn))
      rgb_list.sort()
      all_frames += rgb_list[:-1]
      # train_frame_list_dir = os.path.join(self.dataset_dir, date, 'train_frame_list.txt')
      # with open(train_frame_list_dir) as f:
      #   for line in f:
      #     all_frames.append(date + ' ' + os.path.join(self.dataset_dir,date,line.rstrip()))
  ...
  def is_valid_sample(self, frames, target_index):
    ...
    target_drive, _ = frames[target_index].split(' ')
    ...
    if (target_drive == start_drive and target_drive == end_drive):
      return True
    return False
  ...
  def load_image_sequence(self, frames, target_index):
    ...
    for index in range(start_index, end_index + 1):
      ...
      img = self.load_image_raw(drive, frame_id)
      ...
      # img_w, img_h = Image.fromarray(img).size
      # new_width = math.ceil(self.img_width/self.img_height*img_h)
      # new_image = Image.new('RGB', (new_width, img_h), (255,255,255))
      # new_image.paste(Image.fromarray(img), (0,0,img_w,img_h))
      # img = np.array(new_image.resize((self.img_width, self.img_height)))
      # top = (img_h - self.img_height) / 2
      # img = np.array(Image.fromarray(img).crop((0,top,self.img_width,top+self.img_height)))# .resize((self.img_width, self.img_height))
  ...
  def load_pose_sequence(self, frames, target_index):
    ...
    return [] #don't have pose sequence in nyu
    # target_drive, target_frame_id = frames[target_index].split(' ')
  ...
  def load_example(self, frames, target_index): #TODO: may have problem
    """Returns a sequence with requested target frame."""
    image_seq, zoom_x, zoom_y = self.load_image_sequence(frames, target_index)
    target_drive, target_frame_id = (
        frames[target_index].split(' '))
    intrinsics = self.load_intrinsics_raw(target_drive)
    intrinsics = self.scale_intrinsics(intrinsics, zoom_x, zoom_y)
    example = {}
    example['intrinsics'] = intrinsics
    example['image_seq'] = image_seq
    example['folder_name'] = target_drive + '/'
    example['file_name'] = (target_frame_id.split('/')[-1])[:-4]
    if self.load_pose:
  ...
  def load_intrinsics_raw(self, drive): #TODO: intrinsics parameter need to find where for nyu
    calib_file_path = os.path.join(self.dataset_dir, 'intrinsic.txt')#, drive
    with open(calib_file_path) as f:
      data = {}
      for line in f:
        key, value = line.split(' ')
        data[key] = float(value)
      intrinsics = np.array([[data['fx'], 0,          data['cx']],
                             [0,          data['fy'], data['fy']],
                             [0,          0,          1]])
      return intrinsics
    # date = drive[:10]
    # calib_file = os.path.join(self.dataset_dir, 'calib_cam_intrinsics.txt')
